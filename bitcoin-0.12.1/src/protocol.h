...
public:
    ...
    /*
     * 定义了消息标识,采⽤在正常数据中极少出现的字符,不用大写ASCII码字符,任意对齐,⻓度为4字节,在UTF-8中⽆效。  在不同的环境中,消息开始字符串不同。
    环境类型 消息开始字符串
    主类型(MAIN) 0xd9b4bef9
    ...
    //命令(Command) 12
    /*
     * 定义了通信中的各种命令,长度为12个字节的字符串,由0x20~0x7F之间的字 符串构成,其他字符⽆无效。
     */
    char pchCommand[COMMAND_SIZE];

    //消息⼤大⼩小(MessageSize) 无符号整型
    /*
     * 定义了消息的⼤小,不包含消息头的⼤小,是⽆无符号整型,最⼤大值是32M (0x02000000)。
     */

    unsigned int nMessageSize;
    //校验和(Checksum) 无符号整型
    /*
     * 消息的校验和,不包含消息头的数据,是⽆符号整型。
