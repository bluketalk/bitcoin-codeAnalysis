...
/*
 * 我们和其他节点连通后，就需要进行消息交换，发送和接收数据就是通过这个接口来进行的
 * */
void ThreadSocketHandler()
{
    ...
    while (true)
    {
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy)
        {
            ...
            if (pnode->hSocket == INVALID_SOCKET)
                continue;
            //如果某个节点socket有效，则调用SocketSendData发送节点数据
            ...
            //节点socket连接检测，如果连接时间超过60秒，则进行一次检测
...
void StartNode(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    {
        CAddrDB adb;
        //读取本地的节点文件的数据，并将数据存储在了addrman这个全局对象中
        if (!adb.Read(addrman))
    ...
    //映射upnp设备
    /**
     * UPNP 的英文全称是Universal Plug and Play，即通用即插即用。
     * 是各种各样的智能设备、无线设备和个人电脑等实现遍布全球的对等网络连接（P2P）的结构。
     * 比特币客户端是支持UPNP的，所以需要地址映射，我们现在就看下是如何映射的。
     * 要映射upnp设备是需要miniupnpc.lib这个库，用于映射端口的API是UPNP_AddPortMapping这个接口。
     */
    // Map ports with UPnP
